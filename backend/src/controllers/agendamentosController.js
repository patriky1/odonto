...
exports.criar = (req, res) => {
  try {
    ...
    const r = db.prepare(`INSERT INTO agendamentos
      (pacienteId, dentistaId, procedimentoId, salaId, data, horaInicio, horaFim, status, observacoes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(pacienteId, dentistaId, procedimentoId || null, salaId, data, horaInicio, horaFim || null, status, observacoes || null);

    res.status(201).json(db.prepare(`${SELECT_BASE} WHERE a.id = ?`).get(r.lastInsertRowid));
  } catch (e) {
...
exports.atualizar = (req, res) => {
  try {
    ...
    db.prepare(`UPDATE agendamentos SET pacienteId=?, dentistaId=?, procedimentoId=?, salaId=?, data=?, horaInicio=?,
                horaFim=?, status=?, observacoes=?, updatedAt=datetime('now') WHERE id=?`)
      .run(pacienteId || a.pacienteId, novo.dentistaId,
        ...
        novo.status, observacoes !== undefined ? (observacoes || null) : a.observacoes, id);
...
exports.alterarStatus = (req, res) => {
  ...
  if (!a) return erro(res, 404, 'Agendamento não encontrado');
  if (!podeVerAgendamento(req.usuario, a)) return negado(res);
  db.prepare("UPDATE agendamentos SET status = ?, updatedAt = datetime('now') WHERE id = ?").run(status, id);
