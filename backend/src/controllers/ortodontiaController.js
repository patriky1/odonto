...
exports.criar = (req, res) => {
  try {
    ...
    const b = req.body;
    const trabalhoRealizado = b.trabalhoRealizado?.trim() || null;
    const trabalhoRealizar = b.trabalhoRealizar?.trim() || null;

    // Ficha simplificada (atual): basta um dos dois campos.
    // Compatível com o formulário antigo, que mandava "procedimento".
    if (!trabalhoRealizado && !trabalhoRealizar && !b.procedimento?.trim()) {
      return res.status(400).json({
        erro: 'Preencha ao menos um dos campos de trabalho',
        error: 'Preencha ao menos um dos campos de trabalho',
      });
    }

    // "procedimento" é histórica e NOT NULL no schema; quando o formulário
    // novo não a envia, derivamos um valor pra não quebrar a constraint.
    ...
    const r = db.prepare(`INSERT INTO ortodontia
      (pacienteId, dentistaId, data, tipoAparelho, procedimento, arcada, dentes, fioUtilizado, elasticos,
       forcaAplicada, queixas, orientacoes, proximaConsulta, valor, observacoes,
       trabalhoRealizado, trabalhoRealizar, usuarioId, usuarioNome)
      VALUES (?, ?, COALESCE(?, date('now')), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(pacienteId, b.dentistaId || null, b.data || null, b.tipoAparelho || null,
        procedimento, b.arcada || null, dentes, b.fioUtilizado || null, b.elasticos || null,
        ...
        b.observacoes || null, trabalhoRealizado, trabalhoRealizar,
        req.usuario?.id || null, req.usuario?.nome || null);
...
exports.atualizar = (req, res) => {
  try {
    ...
    const trabalhoRealizado = b.trabalhoRealizado !== undefined
      ? (b.trabalhoRealizado?.trim() || null) : reg.trabalhoRealizado;
    const trabalhoRealizar = b.trabalhoRealizar !== undefined
      ? (b.trabalhoRealizar?.trim() || null) : reg.trabalhoRealizar;
    const procedimento = b.procedimento !== undefined
      ? (b.procedimento?.trim() || trabalhoRealizado || trabalhoRealizar || reg.procedimento)
      : (trabalhoRealizado || reg.procedimento);
    ...
    db.prepare(`UPDATE ortodontia SET dentistaId=?, data=?, tipoAparelho=?, procedimento=?, arcada=?, dentes=?,
                ...
                valor=?, observacoes=?, trabalhoRealizado=?, trabalhoRealizar=?, updatedAt=datetime('now') WHERE id=?`)
      .run(
