...
// Formulário simplificado de ortodontia (ficha estilo "carteirinha")
garantirColunas('ortodontia', {
  trabalhoRealizado: 'TEXT',
  trabalhoRealizar: 'TEXT',
});
 ...
 * Visível só para o admin (ver utils/auditoria.js).
 * ------------------------------------------------------------------ */
...
['agendamentos', 'tratamentos', 'pagamentos', 'receitas', 'despesas'].forEach((t) => garantirColunas(t, COLUNAS_AUDITORIA));

// Receitas e gastos podem ser atribuídos a um dentista (filtro do financeiro)
garantirColunas('receitas', { dentistaId: 'INTEGER' });
garantirColunas('despesas', { dentistaId: 'INTEGER' });

db.exec(`
  -- Log completo de ações (inclui exclusões). Só o admin consulta.
  ...
  -- Termos de autorização / consentimento assinados pelo paciente
  CREATE TABLE IF NOT EXISTS termos_consentimento (
    ...
    status TEXT NOT NULL DEFAULT 'pendente',
    formaAssinatura TEXT,
    assinanteNome TEXT,
    assinanteDocumento TEXT,
    assinanteRelacao TEXT,
...
try {
  ['receitas', 'despesas'].forEach((t) => {
    db.exec(`UPDATE ${t} SET criadoPorId = usuarioId,
               criadoPorNome = (SELECT nome FROM usuarios u WHERE u.id = ${t}.usuarioId)
             WHERE criadoPorId IS NULL AND usuarioId IS NOT NULL`);
  });
} catch (e) {
  console.warn('⚠️  Não foi possível preencher o autor dos lançamentos antigos:', e.message);
}
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_auditoria_data          ON auditoria(createdAt);
  ...
  CREATE INDEX IF NOT EXISTS idx_termos_paciente         ON termos_consentimento(pacienteId);
