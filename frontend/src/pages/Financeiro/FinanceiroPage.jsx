...
function PagamentoForm({ pagamento, pacientes, dentistas, procedimentos = [], onSubmit }) {
  const [form, setForm] = useState({
    pacienteId: pagamento?.pacienteId || '',
    dentistaId: pagamento?.dentistaId || '',
...
function DespesaForm({ despesa, categorias, onSubmit }) {
  const [form, setForm] = useState({
...
function ReceitaForm({ receita, categorias, onSubmit }) {
  const [form, setForm] = useState({
...
export default function FinanceiroPage() {
  ...
  const carregar = useCallback(() => {
    setLoading(true);
    const p = { inicio: periodo.inicio, fim: periodo.fim };
    Promise.all([
      api.get('/financeiro/pagamentos', { params: { ...p, status: filtroStatus || undefined, busca: busca || undefined, dentistaId: filtroDentista || undefined } }),
  ...
  // Limpa filtros específicos ao trocar de aba
  useEffect(() => { setFiltroStatus(''); setFiltroCategoria(''); setBusca(''); setFiltroDentista(''); }, [aba]);
  ...
  const exportarCSV = async () => {
    ...
    try {
      const resp = await api.get('/financeiro/exportar', { params: periodo, responseType: 'blob' });
      const url = URL.createObjectURL(new Blob([resp.data], { type: 'text/csv;charset=utf-8' }));
      const a = document.createElement('a');
      a.href = url;
      a.download = `financeiro_${periodo.inicio}_a_${periodo.fim}.csv`;
    ...
    <div>
      ...
      {aba === 'pagamentos' && (
        <div className="card">
          <div className="search-bar" style={{ gap: 10, flexWrap: 'wrap' }}>
            ...
            <select className="form-control" style={{ maxWidth: 220 }} value={filtroDentista} onChange={(e) => setFiltroDentista(e.target.value)} aria-label="Filtrar por dentista">
                  ...
                  {receitas.map((r) => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td className="td-titulo" style={{ fontWeight: 600 }}>{r.descricao}</td>
                    ...
                    <tr key={r.id} style={r.cancelado ? { opacity: .6 } : undefined}>
                      ...
                      <td data-label="Pagador">
                        ...
                        {r.paciente?.nome && r.paciente.nome !== r.pagadorNome && (
                          <span className="text-xs text-muted" style={{ display: 'block' }}>Paciente: {r.paciente.nome}</span>
                        )}
                      </td>
      ...
      {aba === 'relatorio' && relatorio && liberado && (
        <div style={{ display: 'grid', gap: 16 }}>
          <div className="card">
            <h3 className="card-title mb-4">Demonstrativo do período</h3>
            <div className="table-wrapper">
      ...
      <Modal open={modal === 'pagamento'} onClose={fechar} size="lg"
        ...
        <PagamentoForm key={editando?.id || 'novo'} pagamento={editando} pacientes={pacientes} dentistas={dentistas} procedimentos={procedimentos} onSubmit={salvarPagamento} />
      </Modal>
      ...
      <Modal open={modal === 'despesa'} onClose={fechar} size="lg"
        ...
        <DespesaForm despesa={editando} categorias={categorias.despesas} onSubmit={salvarDespesa} />
      </Modal>
      ...
      <Modal open={modal === 'receita'} onClose={fechar} size="lg"
        ...
        <ReceitaForm receita={editando} categorias={categorias.receitas} onSubmit={salvarReceita} />
      </Modal>
